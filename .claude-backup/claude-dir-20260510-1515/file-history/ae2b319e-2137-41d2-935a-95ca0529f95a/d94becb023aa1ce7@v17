...
| `/shared-docker/adapters/v1_1/` | v1.1 | 2026-05-10 (88 valid entries days 06-09; 18 with corrections) | 0.799 mean train_loss (curve 2.34→~0.39 oscillating) | **Trained but NOT shipped (Outcome C).** rank 16, 29.3 MB. v1.1 lost v1's correction memorization on the demo image (Test A: `Kuman` → `Kumon`, `Suud` → `Suid` — both corrections gone) and broke reading-order coherence on a v1.1-only training image (Test B: RAKtherm sign output fragmented). Cause: 4.5 epochs over 88 examples vs v1's 16 epochs over 25 — corrections diluted by larger ask-mode-heavy dataset at same `max_steps=100`. v1 remains active. Saved as fallback at `data-backup/adapters/v1_1/` for later iteration (filter to corrected entries only, or longer training). Full evidence: `data-backup/comparisons/v1_1-vs-v1.md`. |
